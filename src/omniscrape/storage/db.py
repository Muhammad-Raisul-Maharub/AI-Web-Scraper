# src/omniscrape/storage/db.py - SQLite Persistence, Scrape History, and Price Tracking
import os
import shutil
import sqlite3
import hashlib
...
DEFAULT_DB_FILE = os.path.join(DATA_DIR, "scraper.db")
LEGACY_DB_FILE = os.path.join(PROJECT_ROOT, "scraper.db")

# Automatic safe migration if data/scraper.db doesn't exist yet but root scraper.db does
if not os.path.exists(DEFAULT_DB_FILE) and os.path.exists(LEGACY_DB_FILE):
    try:
        shutil.copy2(LEGACY_DB_FILE, DEFAULT_DB_FILE)
    except Exception:
        pass

DB_FILE = os.getenv("SCRAPER_DB_PATH", DEFAULT_DB_FILE)
